...
bool urllib::URL::parse_scheme(const std::string& uri) {
    m_scheme = "";
...
std::tuple<urllib::URL, urllib::error> urllib::URL::parse(const std::string& raw_url) {
    // Cut off #fragment
    ...
    if (!error.empty()) {
        return std::tuple<URL, urllib::error>({}, {"parse", raw_url});
    }

    if (fragment.empty()) {
        return std::tuple<URL, urllib::error>(url, {});
    }
//    if (url.m_fragment &&
    return std::tuple<URL, urllib::error>(url, {});
}

std::tuple<urllib::URL, urllib::error> urllib::URL::parse_request_URI(const std::string& raw_url) {
    auto[url, err] = parse(raw_url, true);
    if (!err.empty()) {
        return {{},
                {"parse_request_URI", err}};
    };
    return {url, {}};
}

std::tuple<urllib::URL, std::string> urllib::URL::parse(const std::string& raw_url, bool via_request) {
    ...
    auto[scheme, rest, err] = get_scheme(raw_url);
    if (!err.empty()) {
        return {URL(), err};
    }
    ...
    if ((!url.m_scheme.empty() || (!via_request && rest.find("///") != 0)) &&
        ...
        auto[user, host, err1] = parse_authority(authority);
        if (!err1.empty()) {
            return {URL(), err1};
        }
...
std::string urllib::URL::Escape(const std::string& str, urllib::URL::Encode mode) {
...
std::string urllib::URL::SetPath(const std::string& path) {
...
std::tuple<std::string, std::string, std::string> urllib::URL::get_scheme(const std::string& raw_url) {
...
bool urllib::URL::ValidUserInfo(std::string str) {
    return std::all_of(str.cbegin(), str.cend(), [](const char c) {
        return isalnum(c) || std::strchr("-._:~!$&\'()*+,;=%@", c) != nullptr;
    });
}

urllib::URL::user_info urllib::URL::UserNoPassword(const std::string& username) {
    return {username, "", false};
}

urllib::URL::user_info urllib::URL::UserPassword(const std::string& username, const std::string& password) {
    return {username, password, true};
}

std::tuple<urllib::URL::user_info, std::string, std::string>
urllib::URL::parse_authority(const std::string& authority) {
    auto at = authority.find_last_of('@');
    std::string err;
    std::string host;
    if (at == std::string::npos) {
        auto[host1, err1] = parse_host(authority);
        err = std::move(err1);
        host = std::move(host1);
    } else {
        auto[host2, err2] = parse_host(authority.substr(at + 1));
        err = std::move(err2);
        host = std::move(host2);
    }
    if (!err.empty()) {
...
bool urllib::URL::ValidOptionalPort(const std::string& port) {
    if (port.empty()) {
        return true;
    }
    if (port[0] != ':') {
...
std::tuple<std::string, std::string> urllib::URL::parse_host(const std::string& host) {
    if (host.front() == '[') {
...
bool urllib::URL::ShouldEscape(char c, urllib::URL::Encode mode) {
...
std::tuple<std::string, std::string> urllib::URL::Unescape(const std::string& str, urllib::URL::Encode mode) {
